...
class RpcTests(object):
    ...
    def check_blocks(self, object, num_expected):
        ...
        if leaked_blocks != 0:
            print "Leaked {0:d} blocks".format(leaked_blocks)

    def check_type(self, interface, typename, type):
        print "Checking type {0!s}".format(typename)
        v = type()
        for n in dir(v):
            if n[0] == '_':
                continue
            try:
                ...
                if str(errstr) == "unknown union level":
                    print "ERROR: Unknown union level in {0!s}.{1!s}".format(typename, n)
                    self.errcount += 1
                    continue
                print str(errstr)[1:21]
                if str(errstr)[0:21] == "Can not convert C Type":
                    print "ERROR: Unknown C type for {0!s}.{1!s}".format(typename, n)
                    self.errcount += 1
                    continue
                else:
                    print "ERROR: Failed to instantiate {0!s}.{1!s}".format(typename, n)
                    self.errcount += 1
                    continue
            except Exception:
                print "ERROR: Failed to instantiate {0!s}.{1!s}".format(typename, n)
            ...
            try:
                print "Setting {0!s}.{1!s}".format(typename, n)
                setattr(v, n, value)
            except Exception, e:
                if isinstance(e, AttributeError) and str(e).endswith("is read-only"):
                    ...
                    print "ERROR: Failed to set {0!s}.{1!s}: {2!r}: {3!s}".format(typename, n, e.__class__, e)
                    self.errcount += 1
                    continue
            ...
            try:
                if value != getattr(v, n):
                    print "ERROR: Comparison failed for {0!s}.{1!s}: {2!r} != {3!r}".format(typename, n, value, getattr(v, n))
                    continue
            except Exception, e:
                print "ERROR: compare exception for {0!s}.{1!s}: {2!r}: {3!s}".format(typename, n, e.__class__, e)
                continue
    ...
    def check_interface(self, interface, iname):
        ...
        for n in dir(interface):
            ...
            if isinstance(value, str):
                ...
                print "UNKNOWN: {0!s}={1!s}".format(n, value)
        if self.errcount - errcount != 0:
            print "Found {0:d} errors in {1!s}".format(self.errcount - errcount, iname)
    ...
    def check_all_interfaces(self):
        for iname in dir(samba.dcerpc):
            ...
            if iname == 'ClientConnection' or iname == 'base':
                continue
            print "Checking interface {0!s}".format(iname)
...
if errcount == 0:
    ...
    print "{0:d} failures".format(errcount)
    sys.exit(1)
