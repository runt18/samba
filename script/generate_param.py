...
def make_s3_param_proto(path_in, path_out):
    ...
    try:
        ...
        file_out.write("#ifndef {0!s}\n".format(header))
        file_out.write("#define {0!s}\n\n".format(header))
        for parameter in iterate_all(path_in):
            ...
            output_string += "lp_{0!s}".format(parameter['function'])
            ...
            if parameter['type'] == 'string' and not parameter['constant']:
                if parameter['context'] == 'G':
                    ...
                    output_string += '(TALLOC_CTX *ctx, {0!s});\n'.format(param)
                else:
                    raise Exception(parameter['name'] + " has an invalid param type " + parameter['type'])
            else:
                if parameter['context'] == 'G':
...
def make_lib_proto(path_in, path_out):
    ...
    try:
        ...
        for parameter in iterate_all(path_in):
            ...
            output_string += "lpcfg_{0!s}".format(parameter['function'])
...
def get_header(path):
    ...
    return "__{0!s}__".format(header)
...
def make_param_defs(path_in, path_out, scope):
    ...
    try:
        ...
        file_out.write("LOADPARM_EXTRA_{0!s}S\n".format(scope))
        file_out.write("};\n")
        file_out.write("\n#endif /* {0!s} */\n\n".format(header))
    finally:
        file_out.close()
    ...
    try:
        ...
        for parameter in iterate_all(path_in):
            ...
            if parameter['context'] == 'G':
                ...
                temp = "LOCAL"
            offset = "{0!s}_VAR({1!s})".format(temp, parameter['function'])
            ...
            file_out.write("\t{\n")
            file_out.write("\t\t.label\t\t= \"{0!s}\",\n".format(parameter['name']))
            file_out.write("\t\t.type\t\t= {0!s},\n".format(p_type))
            file_out.write("\t\t.p_class\t= {0!s},\n".format(p_class))
            file_out.write("\t\t.offset\t\t= {0!s},\n".format(offset))
            file_out.write("\t\t.special\t= {0!s},\n".format(handler))
            file_out.write("\t\t.enum_list\t= {0!s},\n".format(enumlist))
            if flags != "":
                file_out.write("\t\t.flags\t\t= {0!s},\n".format(flags))
            file_out.write("\t},\n")
