...
class wintest():
    ...
    def setwinvars(self, vm, prefix='WIN'):
        ...
        for v in ['VM', 'HOSTNAME', 'USER', 'PASS', 'SNAPSHOT', 'REALM', 'DOMAIN', 'IP']:
            vname = '{0!s}_{1!s}'.format(vm, v)
            if vname in self.vars:
                self.setvar("{0!s}_{1!s}".format(prefix, v), self.substitute("${{{0!s}}}".format(vname)))
            else:
                self.vars.pop("{0!s}_{1!s}".format(prefix, v), None)
    ...
    def load_config(self, fname):
        ...
        for line in f:
            ...
            colon = line.find(':')
            if colon == -1:
                raise RuntimeError("Invalid config line '{0!s}'".format(line))
    ...
    def substitute(self, text):
        ...
        while True:
            ...
            if not var_name in self.vars:
                raise RuntimeError("Unknown substitution variable ${{{0!s}}}".format(var_name))
            text = text.replace("${{{0!s}}}".format(var_name), self.vars[var_name])
        return text
    ...
    def del_files(self, dirs):
        ...
        for d in dirs:
            self.run_cmd("find {0!s} -type f | xargs rm -f".format(d))
    ...
    def cmd_contains(self, cmd, contains, nomatch=False, ordered=False, regex=False,
        ...
        for c in self.substitute(contains):
            ...
            if nomatch:
                if start != -1:
                    raise RuntimeError("Expected to not see {0!s} in {1!s}".format(c, cmd))
            else:
                if start == -1:
                    raise RuntimeError("Expected to see {0!s} in {1!s}".format(c, cmd))
    ...
    def retry_cmd(self, cmd, contains, retries=30, delay=2, wait_for_fail=False,
        ...
        while retries > 0:
            try:
                ...
                time.sleep(delay)
                retries -= 1
                self.info("retrying (retries={0:d} delay={1:d})".format(retries, delay))
        raise RuntimeError("Failed to find {0!s}".format(contains))
    ...
    def rndc_cmd(self, cmd, checkfail=True):
        '''run a rndc command'''
        self.run_cmd("${{RNDC}} -c ${{PREFIX}}/etc/rndc.conf {0!s}".format(cmd), checkfail=checkfail)
...
'''.format(self.substitute(nameserver)))
        child = self.pexpect_spawn("cat /etc/resolv.conf", crlf=False)
    ...
    def configure_bind(self, kerberos_support=False, include=None):
        ...
        if include and self.getvar('NAMESERVER_BACKEND') != 'SAMBA_INTERNAL':
            self.setvar("NAMED_INCLUDE", 'include "{0!s}";'.format(include))
        else:
...
'''.format(self.getvar('LCREALM'), self.getvar('INTERFACE_IP')),
                     mode='a')
...
''')
    ...
    def restore_resolv_conf(self):
        ...
        if getattr(self, 'resolv_conf_backup', False):
            ...
            self.run_cmd("mv -f {0!s} /etc/resolv.conf".format(self.resolv_conf_backup))
    ...
    def ping_wait(self, hostname):
        ...
        while loops > 0:
            try:
                self.run_cmd("ping -c 1 -w 10 {0!s}".format(hostname))
                break
            except:
                loops = loops - 1
        if loops == 0:
            raise RuntimeError("Failed to ping {0!s}".format(hostname))
        self.info("Host {0!s} is up".format(hostname))

    def port_wait(self, hostname, port, retries=200, delay=3, wait_for_fail=False):
        '''wait for a host to come up on the network'''

        while retries > 0:
            child = self.pexpect_spawn("nc -v -z -w 1 {0!s} {1:d}".format(hostname, port), crlf=False, timeout=1)
        ...
        raise RuntimeError("gave up waiting for {0!s}:{1:d}".format(hostname, port))
    ...
    def set_noexpire(self, child, username):
        ...
        child.sendline('wmic useraccount where name="{0!s}" set PasswordExpires=FALSE'.format(username))
    ...
    def set_ip(self, child):
        ...
        if self.getvar("WIN_IPV4_ADDRESS") != self.getvar("WIN_IP"):
            raise RuntimeError("ipconfig address {0!s} != nmblookup address {1!s}".format(self.getvar("WIN_IPV4_ADDRESS"),
                                                                                self.getvar("WIN_IP")))
    ...
    def resolve_ip(self, hostname, retries=60, delay=5):
        ...
        while retries > 0:
            child = self.pexpect_spawn("bin/nmblookup {0!s}".format(hostname))
            i = 0
            while i == 0:
            ...
            time.sleep(delay)
            self.info("retrying (retries={0:d} delay={1:d})".format(retries, delay))
        raise RuntimeError("Failed to resolve IP of {0!s}".format(hostname))
    ...
    def kinit(self, username, password):
        ...
        if child.exitstatus != 0:
            raise RuntimeError("kinit failed with status {0:d}".format(child.exitstatus))
    ...
    def wait_reboot(self, retries=3):
        ...
        while retries > 0:
            try:
                ...
                self.info("retrying reboot (retries={0:d})".format(retries))
...
''')
    ...
    def run_winjoin(self, vm, domain, username="administrator", password="${PASSWORD1}"):
        ...
        while retries > 0:
            ...
            child.expect("C:")
            child.sendline("netdom join ${{WIN_HOSTNAME}} /Domain:{0!s} /UserD:{1!s} /PasswordD:{2!s}".format(domain, username, password))
    ...
    def test_remote_smbclient(self, vm, username="${WIN_USER}", password="${WIN_PASS}", args=""):
        ...
        self.cmd_contains("{0!s} --version".format((smbclient)), ["${SAMBA_VERSION}"])
        self.retry_cmd('{0!s} -L ${{WIN_HOSTNAME}} -U{1!s}%{2!s} {3!s}'.format(smbclient, username, password, args), ["IPC"], retries=60, delay=5)

    def test_net_use(self, vm, realm, domain, username, password):
        self.setwinvars(vm)
        self.info('Testing net use against Samba3 member')
        child = self.open_telnet("${WIN_HOSTNAME}", "{0!s}\\{1!s}".format(domain, username), password)
        child.sendline("net use t: \\\\${{HOSTNAME}}.{0!s}\\test".format(realm))
        child.expect("The command completed successfully")
    ...
    def setup(self, testname, subdir):
        ...
        if nameserver == self.getvar('NAMED_INTERFACE_IP'):
            raise RuntimeError("old /etc/resolv.conf must not contain {0!s} as a nameserver, this will create loops with the generated dns configuration".format(nameserver))
        ...
        self.setvar('DNS_FORWARDER', "--option=dns forwarder={0!s}".format(nameserver))
